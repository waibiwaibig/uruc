...
- trust policy、conformance 或 risk metadata 的 policy refs
- 紧凑 risk metadata 和 conformance badges

City config 可以在 `federations[federationId]` 下声明 membership，并可提供 document URL 和本地 `trustPolicy`。当前 trust-policy skeleton 可以在 city、issuer、resident 或 domain verification context 中返回 `accept`、`reject`、`warn` 或 `unknown`。它不是法律规则引擎，也不实现全网共识。

Federation policy result 可以附加到 resident/domain/city verification output，作为可审计上下文。它用于后续准入、权限决策、风险标记和 conformance badge 接口。它不删除也不改写 Resident ID。未加入某个 federation 的 city 对该 federation policy context 返回 `unknown`，不需要遵守该 federation policy。Domain attachment/dispatch 与 Venue Domain Protocols 仍然独立于 Federation。
