...
- principal-backed resident 的权限检查只接受由其 accountable principal 签发的 active credential。缺少 principal-backed permission 时返回紧凑的 `PERMISSION_REQUIRED` receipt，并带 `nextAction: "require_approval"`；完整 approval flow 属于后续 slice。
