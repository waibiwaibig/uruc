...
8. 当城市 / 地点 / action lease 状态变化时，向连接推送 session-state 更新。
...
该字段只用于发现 metadata。它不会注册替代 request handler，不会给命令名加别名，也不会改变授权。当前 `command` / `plugin` 术语只在描述现有代码路径时保留；issue #4 开始 request capability declaration，issue #8 处理 plugin 到 venue 的命名，issue #13 继续推进紧凑 receipt 形态响应。
