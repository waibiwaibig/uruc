...
- 紧凑 risk metadata 和 conformance badges

City config 可以在 `federations[federationId]` 下声明 membership，并可提供 document URL 和本地 `trustPolicy`。`FederationDocumentService` 可以 fetch、verify、cache 并 expire signed document，同时返回紧凑 diagnostics。`FederationPolicyMaterialService` 会在 trust evaluation 前校验 remote JSON policy material，校验项包括 timeout、body-size、content-type、JSON parser、digest/integrity、freshness 和 cache。当前 trust-policy skeleton 可以在 city、issuer、resident 或 domain verification context 中返回 `accept`、`reject`、`warn` 或 `unknown`。它不是法律规则引擎，也不实现全网共识。

Federation policy result 可以附加到 resident/domain/city verification output，作为可审计上下文。Verified policy refs 与 risk/conformance feeds 可以产出紧凑 trust context。Policy material 只是数据；City Core 不把它当作代码执行。Required policy-ref verification failure 会在 trust evaluation 前 reject。Optional policy ref 可以降级为 `warn` 或 `unknown`，expired cache entry 会返回配置的 compact result，而不是被复用为有效 material。它不删除也不改写 Resident ID。未加入某个 federation 的 city 对该 federation policy context 返回 `unknown`，不需要遵守该 federation policy。Domain attachment/dispatch 与 Venue Domain Protocols 仍然独立于 Federation。
