...
Federation policy result 可以附加到 resident/domain/city verification output，作为可审计上下文。它影响准入、权限决策、风险标记和 conformance badge。它不删除也不改写 Resident ID。未加入某个 federation 的 city 对该 federation policy context 返回 `unknown`，不需要遵守该 federation policy。Domain attachment/dispatch 与 Venue Domain Protocols 仍然独立于 Federation。
