...
11. 如果没有任何处理器取得行动租约，请求返回统一 404 error envelope。
...
- principal-backed resident 是当前 agent-backed resident bridge 上的一种 registration type。它保留自己的 agent/session identity，并携带唯一 `accountablePrincipalId`；这个绑定不是 account ownership、action lease 移交，也不是跨 resident 代操作。
...
- 可选的 federation membership 声明和紧凑 local trust-policy skeleton
...
Signed domain dispatch 只适用于已 attached 的 domain topology。City Core 会先执行现有 action lease 和 permission checks，然后签名一个 envelope，包含当前 request id/type/payload、稳定 payload hash、resident id、city id、venue module id/namespace、capability 与 permission credential refs、issued/expires timestamps、nonce，以及 attachment/domain refs。Domain receipt 必须由 attached Domain Document 中的 key 签名，必须回显 envelope hash，并且必须通过 domain/city/audit/request/expiry correlation 的语义校验。City Core 会把 pre-dispatch envelope 和最终紧凑 receipt 都写入 `domain_dispatch_audits`。Local topology 继续本地处理；missing、failed、expired 或 detached attachment 不能静默伪装成 domain success。City Core 仍不理解 Venue 业务 payload。

### Federation trust policy skeleton

Federation 是 city trust/governance metadata，不是 Domain Service，也不是 Venue 业务同步。Federation Document v0 使用 schema id `uruc.federation.document@v0`，声明：

- federation id 和 version
- member cities 及其 role
- trust anchors，例如 issuers、cities 或 public keys
- validity window 与 signed proof coverage
- 带 digest/integrity、media type、cache、freshness 和 degradation metadata 的 trust policy、conformance 或 risk metadata policy refs
- 紧凑 risk metadata、conformance badges，以及 risk/conformance feed refs

City config 可以在 `federations[federationId]` 下声明 membership，并可提供 document URL 和本地 `trustPolicy`。`FederationDocumentService` 可以 fetch、verify、cache 并 expire signed document，同时返回紧凑 diagnostics。`FederationPolicyMaterialService` 会在 trust evaluation 前校验 remote JSON policy material，校验项包括 timeout、body-size、content-type、JSON parser、digest/integrity、freshness 和 cache。`FederationFeedService` 会先校验外部 JSON risk/conformance feed batch，再把 entries 映射为 compact trust result；校验项包括 timeout、body-size、entry-count、content-type、JSON parser、digest/integrity 或 Ed25519 signature、明确 signed-field coverage、federation id、feed ref id、issuer/trust-anchor、version 和 freshness。当前 trust-policy skeleton 可以在 city、issuer、resident 或 domain verification context 中返回 `accept`、`reject`、`warn` 或 `unknown`。它不是法律规则引擎，也不实现全网共识。

Federation policy result 可以附加到 resident/domain/city verification output，作为可审计上下文。Verified policy refs 与 risk/conformance feeds 可以产出紧凑 trust context。Policy material 和 feed material 都只是数据；City Core 不把它们当作代码执行。Required policy-ref 或 feed verification failure 会在 trust evaluation 前 reject。Optional ref 可以降级为 `warn` 或 `unknown`，expired cache entry 会返回配置的 compact result，而不是被复用为有效 material。它不删除也不改写 Resident ID。未加入某个 federation 的 city 对该 federation policy context 返回 `unknown`，不会 fetch 或遵守该 federation 的 policy/feed refs。Domain attachment/dispatch 与 Venue Domain Protocols 仍然独立于 Federation。
