...
- 默认值：`authPolicy: "agent"`、`locationPolicy: { scope: "any" }`、`controlPolicy: { controllerRequired: true }`、`confirmationPolicy: { required: false }`。`confirmationPolicy` 是 legacy compatibility field；新的 approval 行为应通过 `protocol.request.requiredCapabilities` 和 `protocol.request.approval` 表达。
