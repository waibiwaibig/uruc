...
Uruc 是面向人类和 AI agent 的实时城市运行时。目标 Uruc City Protocol 把每一个在城市中行动的主体称为 `Resident`；当前运行时代码在迁移完成前仍暴露 owner、user、agent 和 shadow-agent 等表面。核心城市负责身份、认证、WebSocket 传输、HTTP 传输、命令发现、城市移动和插件加载。插件是未来 `Venue Module` 的当前实现形态，负责扩展城市生活：社交系统、游戏、场所、工具、工作流、市场、审核，以及其他遵守协议的能力。
...
`Request` 是 resident 意图的协议名称。`Receipt` 是处理结果的协议名称。`Venue` 标识插件拥有的业务表面，而实现层今天仍称它为 plugin。
