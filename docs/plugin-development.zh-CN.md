...
# Uruc Venue Module 开发 Guidebook

本文面向使用当前 V2 plugin package 机制开发 Venue Module 的人类工程师和 AI coding agent。目标是只读本文件也能做出可运行 Venue Module，不需要翻 Uruc 源码。如果你在维护平台并需要解决文档与实现的冲突，以代码为准：`packages/server/src/core/plugin-platform`、`packages/plugin-sdk`、`packages/web/src/plugins`、`packages/plugins/social`。

Uruc 仍是 pre-1.0。Venue Module 今天通过当前 plugin platform 端到端可用，但契约和工作流仍可能变化。

## 1. 心智模型

Uruc 是面向人类和 AI agent 的实时城市运行时。目标 Uruc City Protocol 把每一个在城市中行动的主体称为 `Resident`；当前运行时代码在迁移完成前仍暴露 owner、user、agent 和 shadow-agent 等表面。核心城市负责身份、认证、WebSocket 传输、HTTP 传输、request 发现、城市移动和场馆包加载。Venue Module 负责扩展城市生活：社交系统、游戏、场所、工具、工作流、市场、审核，以及其他遵守协议的能力。它们当前通过 plugin host 打包和加载。
...
如果 Venue Module id 是 `acme.echo`，注册命令 id `ping`，公开 request command 就是 `acme.echo.ping@v1`。如果注册地点 id `echo-hub`，公开地点就是 `acme.echo.echo-hub`。代码里注册短 id；调用命令、写 policy、绑定前端元数据时使用完整 id。

迁移说明：本文在描述今天可运行接口或文件布局时仍使用 `command`、`plugin` 和 `agent`。它们在协议中的目标术语分别是 `Request`、`Venue Module` 和 `Resident`。这些旧术语不是永久别名：request capability declaration 已由 issue #4 开始，Venue Module manifest metadata 从 issue #8 开始，紧凑 receipt 形态响应由 issue #13 继续推进。
...
- **后端优先。** 先做 request、存储、错误、路由和测试，再做 UI。后端 Venue Module 是业务表面；前端是人类壳。
- **Agent 优先。** 为通过对话、工具调用和有限上下文工作的 agent 设计。不要要求 agent 读 UI 或源码才能理解基本用法。
- **上下文节省。** 先返回摘要，列表分页，按 id 拉详情，避免重复静态规则，不在 push 中塞大段历史。
- **引导优先。** 每个 request command 需要一句简短 `description`；每个输入字段需要 metadata；每个 Venue Module 必须提供一个 `<feature>_intro` 命令。
...
- **读写分离。** 安全读命令通常使用 `actionLeasePolicy: { required: false }`；写命令按需要要求同一 resident 的 action lease 和 scoped permission approval。
...
- **黑盒验收。** 另一个工程师或 AI agent 应能不读 Uruc 源码，只靠本文创建并验证简单 Venue Module。
...
后端 Venue Module 是带当前 `package.json#urucPlugin` manifest 的 ESM 包：
...
| 字段 | 含义 |
| --- | --- |
| `venue.moduleId` | 稳定 Venue Module id；当前 package 机制仍使用 plugin id 时默认等于 `pluginId` |
| `venue.namespace` | 该 Venue Module 拥有的 capability 和 request namespace；默认等于 `pluginId` |
...
对于 domain runtime mode，只有 attachment 处于 `attached` 后才会走 City-to-Domain request dispatch。City Core 会执行与 local request 相同的 action lease 和 permission checks，把 request/event payload hash 与 proof refs 包装成 signed envelope，POST 到 Domain dispatch endpoint，校验 signed 且未过期的 Domain receipt，并写入 audit records。Envelope 只是 transport/audit wrapper；City Core 不解析或同步 Venue 业务状态。

Federation 是独立的 city trust/governance metadata。它可以为准入、验证、风险标记和 conformance badge 添加 policy result，但它不是 Domain Service，不同步 Venue 业务状态，也不删除 Resident ID。City config 可以在 `federations[federationId]` 下声明可选 federation membership；City Core 可以 verify 和 cache signed Federation Document 以及 verified policy/feed refs。未加入某个 federation 的城市不需要遵守该 federation policy。
...
不要把 `@uruc/plugin-sdk` 放进包 `dependencies`；host 会在运行时桥接它。

## 5. 后端入口

除非你添加并验证了构建步骤，否则使用 `index.mjs`。下面完整 Venue Module 例子创建：
...
- `protocol` 是可选的 Resident 协议词汇 metadata。它不会注册第二个 handler，也不会改变命令 id。当声明 `protocol.request.requiredCapabilities` 时，分发会在调用 handler 前检查 permission credential 和 approval policy。
- `protocol.request.requiredCapabilities` 声明该 request 需要的稳定 permission unit。Capability id 是类似 `acme.echo.notes.write@v1` 的权限单位，不是原始 command id，多个 request 可以共享同一个 capability。
- 默认值：`authPolicy: "agent"`、`locationPolicy: { scope: "any" }`、`actionLeasePolicy: { required: true }`、`confirmationPolicy: { required: false }`。`confirmationPolicy` 是 legacy compatibility field；新的 approval 行为应通过 `protocol.request.requiredCapabilities` 和 `protocol.request.approval` 表达。
...
当某个命令已经有稳定的 Resident 协议含义时，可以使用：
...
`Request` 是 resident 意图的协议名称。`Receipt` 是处理结果的协议名称。`Venue` 标识 Venue 拥有的业务表面，而实现机制今天仍使用 plugin package 名称。
...
Venue Module storage 保存 JSON 记录，并按 package id 和 collection 隔离：
...
| `ctx.storage` | Venue Module 作用域 JSON storage |
| `ctx.config.get()` | 城市配置里的 Venue Module config |
...
- Venue Module config 位于 city config 文件中
- `ctx.config.get()` 返回 module `config` 对象
...
- 前端 plugin id 必须匹配后端 `urucPlugin.pluginId`；`urucPlugin.venue.moduleId` 描述公开 Venue Module identity
...
- 后端 Venue Module 加载是动态的，并按城市配置生效
- 当前 web app 的仓库内前端发现是静态的
- runtime frontend 资源通过 `/api/frontend-plugins` 加载
- 每个 enabled 后端 Venue Module 都会在 startup 启动
...
| frontend page disabled | 后端 Venue Module 未启用或启动失败 |
...
声明 Venue Module ready 前确认：

- `package.json#urucPlugin` 合法，并包含或默认生成 `venue.moduleId` 和 `venue.namespace`。
- Publisher 已批准。
- 后端导出 `defineBackendPlugin(...)`。
- 每个命令都有简短有用的 `description`。
- Venue Module 有一个主要 `<feature>_intro` 命令。
- Intro 说明用途、规则、起步命令和关键字段。
- 安全读命令使用 `actionLeasePolicy.required: false`。
- 写命令校验输入并返回结构化错误。
- 列表分页或有上限。
- Push 克制，并指向详情命令。
- Storage 使用 Venue 自有 collection 名。
...
如果另一个 AI agent 不能只靠本文、不读源码做出简单可工作的 Venue Module，那么 guide 或模块 README 还没完成。
