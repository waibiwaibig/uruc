...
本文记录 PR [#42](https://github.com/waibiwaibig/uruc/pull/42) 合并后的
Uruc Protocol v1 发布准备状态。对应 merge commit 是
`b1735c954da6218a0f476c1a76482e39ff1265e1`。

下面状态基于该 merge 后当前 `main` 分支、2026-05-06 核对到的 GitHub
...
- Federation risk 和 conformance feed verification，以及 compact trust results。
- protocol conformance tests，以及 resident-facing API compatibility cleanup。
- real-run smoke coverage 已包含 permission-required live fixture。
- v1 baseline 的 federation hardening 已完成。
...
v1 baseline 包含 [#32](https://github.com/waibiwaibig/uruc/issues/32) 和
[#33](https://github.com/waibiwaibig/uruc/issues/33) 跟踪的 federation policy
reference integrity 与 risk/conformance feed verification 工作。
[#26](https://github.com/waibiwaibig/uruc/issues/26) hardening umbrella 已关闭。
...
- federation 之间的全局共识
- federation/domain 处理 Venue business synchronization
...
expiry、policy reference integrity checks、risk feed verification、conformance feed
verification，以及 compact trust context。它不实现 global consensus、legal policy
execution、federation/domain 处理 Venue business synchronization，也不通过 federation
改写 Resident ID。

## Issue 和 PR 状态

以下状态已在 2026-05-06 通过 GitHub 核对：
...
#26 已关闭，作为 v1 baseline 已完成的 hardening/migration umbrella。#32 和
#33 已关闭，作为 v1 baseline 已完成的 federation hardening 工作。

## 推荐下一步

该发布准备状态之后仍保留的工作：

1. 编写 production deployment runbook。
2. 准备 external ecosystem onboarding。
3. 自动化长期 conformance badge 发布。
