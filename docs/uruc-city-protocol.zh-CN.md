...
| Action lease / 行动租约 | 同一居民多客户端或多运行时时，用于限制写入提交的 same-resident session lease。它不是身份控制权。 |
...
每个行动都只由一个 acting resident 签名或提交。任何居民都不能操作、冒充、接管或静默代签另一个居民。
...
当前 controller 概念不应成为身份概念。

未来 primitive 是 same-resident action lease：
