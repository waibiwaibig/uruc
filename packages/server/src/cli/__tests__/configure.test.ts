...
const mocks = vi.hoisted(() => ({
  ...
  getBundledPluginPresetState: vi.fn((preset: string, current?: Record<string, boolean>) => {
    const defaultState = {
      ...
      'uruc.chinese-chess': current?.['uruc.chinese-chess'] ?? true,
      'uruc.chess': current?.['uruc.chess'] ?? true,
      'uruc.example': current?.['uruc.example'] ?? true,
      'uruc.go': current?.['uruc.go'] ?? true,
  ...
  detectBundledPluginState: vi.fn((config: any) => ({
    ...
    'uruc.chinese-chess': config.plugins?.['uruc.chinese-chess']?.enabled ?? false,
    'uruc.chess': config.plugins?.['uruc.chess']?.enabled ?? false,
    'uruc.example': config.plugins?.['uruc.example']?.enabled ?? false,
    'uruc.go': config.plugins?.['uruc.go']?.enabled ?? false,
...
vi.mock('../lib/city.js', () => ({
  BUNDLED_PLUGINS: [
    ...
    { pluginId: 'uruc.chinese-chess', packageName: '@uruc/plugin-chinese-chess', packageDir: 'chinese-chess', label: 'Chinese Chess' },
