...
export function ChessPage() {
  ...
  const disconnectedPlayers = useMemo(
    () => boardMatch?.players.filter((player) => player.disconnectDeadlineAt) ?? [],
    [boardMatch],
  );
  ...
  const statusHighlights = useMemo(() => {
    ...
    if (matchState.drawOfferBy) {
      notes.push(t('chess.page.drawOffered'));
    }
    if (matchState.inCheck) {
      notes.push(t('chess.page.checkShort'));
    }
    for (const player of disconnectedPlayers) {
      const secondsLeft = Math.max(0, Math.ceil((player.disconnectDeadlineAt! - Date.now()) / 1000));
      notes.push(t('chess.page.playerOffline', { name: player.agentName, seconds: secondsLeft }));
    }
    return notes;
  }, [boardMatch, disconnectedPlayers, t]);
  ...
  const stageStatusChips = useMemo(() => {
    const match = boardMatch;
    if (!match) {
      return [
        t('chess.page.timeControl'),
        t('chess.page.syncChip', { value: syncLabel }),
      ];
    }

    if (match.phase === 'waiting') {
      return [
        t('chess.page.phaseChip', { value: match.phase }),
        t('chess.page.playersCount', { count: match.players.length }),
        workspaceTab === 'rooms' && inspectedRoom
          ? t('chess.page.spectatorChip', { count: inspectedRoom.summary.spectatorCount })
          : null,
      ];
    }
    ...
    return [
      match.result
        ? t('chess.page.resultChip', { value: formatResultTitle(match.result) })
        : yourTurn
          ? t('chess.page.turnChip', { value: t('chess.page.yourTurn') })
          : t('chess.page.turnChip', { value: formatTurnLabel(match.turn) }),
      boardDerived.lastMove
        ? t('chess.page.lastMove', { from: boardDerived.lastMove.from, to: boardDerived.lastMove.to })
        : null,
      workspaceTab === 'rooms' && inspectedRoom
        ? t('chess.page.spectatorChip', { count: inspectedRoom.summary.spectatorCount })
        : null,
    ];
  }, [boardDerived.lastMove, boardMatch, inspectedRoom, syncLabel, t, workspaceTab, yourTurn]);
  ...
  const sidebarMeta = (
    <div className="chess-sidebar-meta">
      <span className="chess-visually-hidden">{t('chess.page.boardTitle')}</span>
      {!boardMatch ? <span className="chess-visually-hidden">{t('chess.page.noMatch')}</span> : null}
      <div className="chess-stage-badges chess-stage-badges--sidebar">
        {stageStatusChips.filter(Boolean).map((chip) => (
          <span key={chip} className="chess-stage-badge">{chip}</span>
        ))}
        {statusHighlights.map((chip) => (
          <span key={chip} className="chess-stage-badge chess-stage-badge--warn">{chip}</span>
        ))}
      </div>
      {boardMatch?.result ? (
        <div className="result-banner">
          <div className="result-banner__head">
          ...
          {boardMatchIsCurrent && boardMatch.yourAgentId && typeof boardMatch.result.ratingChanges[boardMatch.yourAgentId] === 'number' ? (
            <div className="result-banner__delta">
